...
variable "sourceimage" {
  type    = string
  default = "/usr2/home/jay/src/terraform-t2/sources/ubuntu.qcow2"
}
...
resource "libvirt_volume" "ctl_qcow2_volume" {
  count          = 3 
  name           = "${each.value.name}.qcow2"
  base_volume_id = libvirt_volume.baseosimage.id
  pool           = var.diskpool
  format         = "qcow2"
  size           = var.ctlplanedisk
}

# Use cloudinit config file and forward some variables to cloud_init.cfg
data "template_file" "ctl_user_data" {
  template   = file("${path.module}/cloud_init.cfg")
  count      = 3
  hostname   = var.ctlplanenames[count.index]
  domainname = var.domainname  
}

# Use CloudInit to add the instance
resource "libvirt_cloudinit_disk" "commoninit" {
  name      = "commoninit_${var.ctlplanenames[count.index]}.iso"
  user_data = data.template_file.user_data[each.key].rendered
}

# Define KVM-Guest/Domain
resource "libvirt_domain" "ctlplanevm" {
  count  = 3
  name   = var.ctlplanenames[count.index] 
  memory = var.ctlplanememory
  vcpu   = var.ctlplanevcpu
...
output "hostnames" {
  value = [libvirt_domain.ctlplanevm.*]
}